...
// Get all active subject names (lightweight for dropdowns)
router.get("/names", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to fetch subject names" });
  }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Failed to create course notification:", err);
    }
